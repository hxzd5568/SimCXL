...
"""
SimCkptDevice P3 functional test: VM driver + user-space stack (Ruby).

Boots a single x86 VM, switches KVM -> Timing, and runs ckptbench, which drives
the SimCkptDevice through libckpt + ckptd (manifest / pinned pool). libckpt
falls back to /dev/mem + pagemap when the simckpt.ko module is not loaded, so
the exact same uapi ABI and save/restore path are exercised.
"""
...
command = (
    ...
    + "echo 'enable failed';"
    + "/home/test_code/ckptbench;"              # P3 user-space stack test
    + "m5 exit;"                                # end
)
...
print("Running SimCkptDevice P3 user-space stack test...")
m5.stats.reset()
simulator.run()
