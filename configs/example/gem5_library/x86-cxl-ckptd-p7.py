...
"""
SimCkptDevice P7 test: GPU payload realism + data integrity (Ruby).

Boots a single x86 VM, switches KVM -> Timing, and runs ckptbench_p7, which
emulates the GPU DMA engine staging a reproducible payload into DRAM and CXL,
saves to storage, restores, and verifies the full round trip with per-chunk
CRC32 + full-checkpoint SHA-256 (acceptance #1, #5).
"""
...
print("Running SimCkptDevice P7 (GPU payload + data integrity) test...")
m5.stats.reset()
simulator.run()
