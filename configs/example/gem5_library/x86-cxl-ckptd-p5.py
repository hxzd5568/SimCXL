...
"""
SimCkptDevice P5 test: CXL hot standby (Ruby).

Boots a single x86 VM, switches KVM -> Timing, and runs ckptbench_p5, which
saves a checkpoint to storage, keeps a hot subset in CXL, then restores it at
hit rates 0/25/50/100% and reports the restore time (acceptance #4).
"""
...
print("Running SimCkptDevice P5 (hot standby) test...")
m5.stats.reset()
simulator.run()
