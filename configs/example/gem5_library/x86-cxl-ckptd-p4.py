...
"""
SimCkptDevice P4 test: dynamic lane routing + pressure control (Ruby).

Boots a single x86 VM, switches KVM -> Timing, and runs ckptbench_p4, which
exercises the ckptd balancer (DRAM/CXL lane routing via predicted finish time +
headroom) and the pinned-pool pressure state machine (NORMAL/FROZEN/SHRINK/
STOPPED). Optionally runs a single-shot bandwidth save (--save-mib/--mode) whose
throughput is read from the device execTicks stat.
"""
...
_parser.add_argument("--save-mib", type=int, default=0)
_parser.add_argument("--mode", type=str, default="dram",
                     choices=["dram", "cxl", "balanced"])
...
_bench = f" --save-mib {_args.save_mib} --mode {_args.mode}" if _args.save_mib else ""

command = (
    "m5 exit;"
    + "echo 1 > /sys/bus/pci/devices/0000:00:07.0/enable 2>/dev/null || "
    + "echo 'enable failed';"
    + "/home/test_code/ckptbench_p4" + _bench + ";"
    + "m5 exit;"
)
...
print("Running SimCkptDevice P4 (routing + pressure) test...")
m5.stats.reset()
simulator.run()
