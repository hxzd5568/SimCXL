...
"""
SimCkptDevice P6 test: application-layer LLM training save/fault/recover (Ruby).

Runs ckptbench_p6 twice: first `train` (with a fault injected mid-checkpoint),
which crashes and persists the manifest, then `recover`, which restores the last
COMMITTED generation and verifies it byte-for-byte.
"""
...
command = (
    ...
    + "echo 'enable failed';"
    + "/home/test_code/ckptbench_p6 train mid;"   # save + fault + crash
    + "/home/test_code/ckptbench_p6 recover;"     # restore + verify
    + "m5 exit;"
)
...
print("Running SimCkptDevice P6 (LLM train save/fault/recover) test...")
m5.stats.reset()
simulator.run()
